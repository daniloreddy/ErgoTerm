...
def test_get_server_status_success(client, m):
    m.post(
        ...
        json={"success": True, "version": "v1.0", "users": {"total": 5}},
        status_code=200,
    )
...
def test_get_server_status_api_error(client, m):
    m.post(
        f"{client.base_url}/v1/status",
        json={"success": False, "error": "Maintenance Mode"},
        status_code=200,
    )
    result = client.get_server_status()
...
def test_check_auth_success(client, m):
    m.post(
        f"{client.base_url}/v1/check_auth",
        json={"success": True, "accountName": "TestUser"},
        status_code=200,
    )
...
def test_check_auth_failure(client, m):
    m.post(f"{client.base_url}/v1/check_auth", json={"success": False}, status_code=200)
    result = client.check_auth("user", "wrongpass")
    assert result["success"] is False
...
def test_get_account_details_success(client, m):
    ...
    m.post(
        f"{client.base_url}/v1/account_details", json=expected_response, status_code=200
    )
...
def test_get_account_details_not_found(client, m):
    m.post(
        f"{client.base_url}/v1/account_details",
...
def test_rehash_server_failure(client, m):
    m.post(
        f"{client.base_url}/v1/rehash",
        json={"success": False, "error": "Permissions Denied"},
        status_code=200,
    )
    result = client.rehash_server()
    assert result["success"] is False
...
def test_register_account_success(client, m):
    m.post(
        f"{client.base_url}/v1/saregister",
        json={"success": True, "errorCode": "OK"},
        status_code=200,
    )
...
def test_register_account_exists_error(client, m):
    m.post(
        f"{client.base_url}/v1/saregister",
        json={"success": False, "errorCode": "ACCOUNT_EXISTS"},
        status_code=200,
    )
...
def test_account_list_success(client, m):
    ...
    m.post(
        f"{client.base_url}/v1/account_list", json=expected_response, status_code=200
    )
